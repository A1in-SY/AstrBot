...
当前维护基线为官方稳定 tag `v4.27.2`，当前 A1in source release 为 `a1in-v4.27.2.4`。
...
| A1in 发行版本 | `astrbot/a1in_release.py` 的 `A1IN_RELEASE` | A1in source release 的唯一源码内身份 | `a1in-v4.27.2.4` |
| A1in 发行修订号 | `A1IN_RELEASE_REVISION` | 同一官方基线内的 A1in release 序号 | `4` |
...
| `dist/assets/a1in-release` | A1in release tag，例如 `a1in-v4.27.2.4` | 构建产物身份、排障和持久化 Dashboard 缓存校验 |
...
5. 将 release branch 合并到 `master`，创建 annotated tag，例如 `a1in-v4.27.2.4`，并将 `master` 和 tag 推送到 `origin`。
...
release 流程必须验证：Git tag、`A1IN_RELEASE`、`A1IN_RELEASE_REVISION`、`A1IN_UPSTREAM_BASE_TAG` 和官方基线 tag 一致。推送 `master` 与 annotated tag 后，`.github/workflows/a1in-release-image.yml` 会由 `a1in-v*` tag push 自动构建并推送 GHCR 归档镜像（human image tag 与 `sha-<short-sha>`）；workflow 只接受 `a1in-vX.Y.Z.N` 格式的 annotated tag，不发布 `latest`，也不参与默认生产发布。校验时从官方只读仓库精确抓取声明的稳定 tag 到隔离 ref，不要求 A1in `origin` 复制官方 tag。

`a1in-v4.27.2.1` 已作为不可变失败发布证据保留：其归档工作流因旧校验错误地要求 A1in `origin` 包含官方 `v4.27.2` tag 而在镜像构建前终止，没有对应 GHCR 镜像；修复不得移动或重写该 tag，后续发布使用 `a1in-v4.27.2.2`。
...
- 固定证据契约、逐图结构化输出与校验/fallback 语义统一的视觉分析内核。
