...
| A1in 发行版本 | `astrbot/a1in_release.py` 的 `A1IN_RELEASE` | A1in source release 的唯一源码内身份 | `a1in-v4.26.8.8` |
| A1in 发行修订号 | `A1IN_RELEASE_REVISION` | 同一官方基线内的 A1in release 序号 | `8` |
...
| --- | --- | --- |
| 官方 `v4.26.8` 上的第 8 个 A1in release | `a1in-v4.26.8.8` | `v4.26.8-a1in.8` |
...
| `dist/assets/a1in-release` | A1in release tag，例如 `a1in-v4.26.8.8` | 构建产物身份、排障和持久化 Dashboard 缓存校验 |
...
5. 将 release branch 合并到 `master`，创建 annotated tag，例如 `a1in-v4.26.8.8`，并将 `master` 和 tag 推送到 `origin`。
