...
@pytest.mark.asyncio
async def test_astrbot_updater_rejects_official_updates_for_a1in_release(
    ...
    with pytest.raises(A1inOfficialUpdateDisabledError):
        await updater.check_update(False)
...
async def test_astrbot_updater_ensure_dashboard_uses_compatible_managed_assets(
    ...
    monkeypatch.setattr(
        core_updater,
        "_is_dist_compatible",
        lambda path, _version, _expected_a1in_release=None: path == data_dist,
    )
...
async def test_astrbot_updater_ensure_dashboard_downloads_matching_assets(
    ...
    monkeypatch.setattr(
        core_updater,
        "_is_dist_compatible",
        lambda path, _version, _expected_a1in_release=None: (
            path == data_dist and (path / "index.html").is_file()
        ),
    )
    monkeypatch.setattr(
        core_updater,
