...
def build_skills_prompt(skills: list[SkillInfo]) -> str:
    ...
    ``name`` and ``description`` are shown upfront; the LLM must use the
    request-scoped ``load_skill`` tool before executing a Skill (progressive
    disclosure).
    """
    ...
    return (
        ...
        'first call `load_skill(name="<skill_name>")` for that exact name. '
        "The tool returns the full `SKILL.md` instructions. "
