...
def _read_dashboard_a1in_release(dist_dir: str | Path) -> str | None:
    """Read the optional A1in release marker from Dashboard assets.

    Args:
        dist_dir: Dashboard dist directory path.

    Returns:
        Release from ``assets/a1in-release``, or None when it is absent.
    """
    ...
    try:
        ...
        logger.warning(
            "Failed to read A1in Dashboard release from %s: %s",
            release_file,
            exc,
        )
    return None
...
def _is_dist_compatible(
    ...
    Args:
        dist_dir: Dashboard dist directory path.
        current_version: Current AstrBot Core version.
        expected_a1in_release: Optional A1in release marker required from the
            Dashboard assets.
    ...
    return (
        (dist_path / "index.html").is_file()
        and _is_version_compatible(
            _read_dashboard_version(dist_path),
            current_version,
        )
        and (
            expected_a1in_release is None
            or _read_dashboard_a1in_release(dist_path) == expected_a1in_release
        )
    )
...
def _should_use_bundled_dist(user_dist: str | Path, current_version: str) -> bool:
    ...
    if not user_dist.exists() or not _is_dist_compatible(
        ...
        return False
    return not _is_dist_compatible(
        user_dist,
        current_version,
        _read_dashboard_a1in_release(bundled_dist),
    )
...
def resolve_dashboard_dist(webui_dir: str | Path | None = None) -> Path | None:
    ...
    bundled_a1in_release = _read_dashboard_a1in_release(bundled_dist)
    if user_dist.exists() and _is_dist_compatible(
        user_dist,
        VERSION,
        bundled_a1in_release,
    ):
